...
async function getLeftoverData(date, success, fail) {
    await api.get(`/leftovers/analysis`, {params: date}).then(success).catch(fail);
}

// FastAPI로 사진을 보내는 POST 요청이 필요합니다
async function sendLeftoverImg(leftover, success, fail) {
    const formData = new FormData();
    formData.append("board", new Blob([JSON.stringify(leftover)], { type: "application/json" }))
...
async function sendLeftoverData(leftover, success, fail) {
    await api.post(`/leftovers/register`, JSON.stringify(leftover)).then(success).catch(fail);
}


export { dailyRank, getLeftoverData, sendLeftoverImg, sendLeftoverData }